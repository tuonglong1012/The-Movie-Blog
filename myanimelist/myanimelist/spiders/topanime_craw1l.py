...
class TopanimeCraw1lSpider(scrapy.Spider):
    ...
    def __init__(self, myarg=None, **kwargs):
        ...
        if myarg:
            self.myarg = ast.literal_eval(myarg)
        else:
            self.arg1 = []
    ...
    def parse(self, response):
        ...
        for url in urls:
            ...
            anime_id = url.css(".anime_ranking_h3 a::attr(href)").get().split("/")[-2]

            if anime_id and anime_id not in self.myarg:
                yield response.follow(
                    url.css(".anime_ranking_h3 a::attr(href)").get(),
                    self.parse_anime,
                    meta={
    ...
    def parse_anime(self, response):
        ...
        for character in characters:
            ...
            character_name = character.css(".h3_characters_voice_actors a::text").get()
            ...
            test["characters"][f"{character_name}"] = {
                ...
                f"{character_name}_path": (
                    f"characters_{id}_{re.sub(
                    r'[^\w\-_]', '_', character_name)}.jpg"
                    if character_name
                    else None
                ),
                f"{character_name}_voice_actor_link": (
                    f"characters_{id}_{re.sub(
                    r'[^\w\-_]', '_', character_name)}_voiceActor.jpg"
                    if character_name
                    else None
                ),
            }
